...
import { BookingsProps } from "../../../Typings/route";
...
const Bookings: FC<BookingsProps> = ({ navigation, route }) => {
      ...
      <View style={styles.imageTextContainer}>
        ...
        >
          ...
          <View style={{ gap: verticalScale(18) }}>
            ...
            >
              State :{" "}
              {item.paymentStatusFromUser === "paid" ? "Paid" : "Pending"}
            </CustomText>
          </View>
        </View>
        <CustomText
  ...
  const renderPastBookingList = ({
    ...
    <View key={index} style={styles.listContainer}>
      <View style={styles.imageTextContainer}>
        <Image
          source={{ uri: item.eventId.media.coverPhoto }}
          style={styles.imgStyle}
        />
        <View
          style={{
        ...
        >
          ...
          <View style={{ gap: verticalScale(18), flex: 1 }}>
            <CustomText
              fontSize={12}
              fontFamily="regular"
              color={COLORS.greyMedium}
  ...
  const handleAcceptRequest = async (bookingId: string) => {
    try {
      const acceptResponse = await patchData(ENDPOINTS.acceptBookingRequest, {
        bookingId: bookingId,
      });
  ...
  const handleRejectRequest = async (bookingId: string) => {
    try {
      const rejectResponse = await patchData(ENDPOINTS.rejectBookingRequest, {
        bookingId: bookingId,
      });

      if (rejectResponse.data.success) {
        // Remove the rejected booking from the pending list
        setPendingBookings((prevBookings) =>
          prevBookings.filter((booking) => booking._id !== bookingId),
        );
        showCustomToast("success", "Booking request rejected successfully.");
      }
    } catch (error) {
  ...
  const renderPendingList = ({
    ...
    item: GetPendingBookingApiResponse;
    index: number;
  }) => (
    <TouchableOpacity
      key={index}
    ...
    >
      ...
      <View style={styles.acceptRejectWrapper}>
        <TouchableOpacity
        ...
        >
          <CustomText fontFamily="bold" color={COLORS.white} fontSize={14}>
            Accept
          </CustomText>
        </TouchableOpacity>
      </View>
  ...
  const handleFetchBoookings = async () => {
    try {
      const [pendingRes, completedRes] = await Promise.all([
        fetchData<GetPendingBookingApiResponse[]>(
          ENDPOINTS.getPendingBookingRequest,
      ...
      console.log("Pending:", pendingRes.data.data);
